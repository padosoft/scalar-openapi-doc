...
- **[claude/permission-precedence]** In `.claude/settings.json`, rules evaluate **deny → ask → allow**: a `deny` always wins, so you **cannot** allow-list an exception to a broad deny glob. A broad `Read(./.env.*)` deny blocks `.env.example`/`.env.testing` too, and re-adding them under `allow` does nothing. **Why:** an automated security review suggested "broad `.env.*` deny + narrow allow for safe files" — technically impossible here. **How to apply:** to protect secret env files while keeping the template/test env readable, enumerate the secret-bearing patterns by Laravel convention (`.env`, `.env.local`, `.env.*.local`, `.env.<environment>`, `.env.backup/.bak`) rather than a catch-all glob. Note `Read(...)` deny does not stop `Bash(cat .env)` — but `cat`/`Get-Content` aren't in the allow list, so they prompt anyway.
